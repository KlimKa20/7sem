...
class Statistics:
    ...
    def __init__(self, _lambda, mu, v, m, n, data):
        ...
        self.__stat_free = np.array(data[1])
        self.__queue_list = np.array(data[2])
        self.__total_request = np.array(data[3])
        self.__queue_time = np.array(data[4])
        self.__total_time = np.array(data[5])
    ...
    def get_e_prob(self):
        return len(self.__stat_free[self.__stat_free == 0]) / len(self.__stat_free), len(
            self.__stat[self.__stat == 2]) / len(self.__stat)
    ...
    def avg_total_t(self, p0, pn):
        ...
        return sum([index * p0 * (teta ** index) / math.factorial(index) for index in range(1, self.__n + 1)]) + sum(
            [(self.__n + index) * pn * teta ** index / np.prod(np.array([self.__n + l * beta for l in range(1, index + 1)])) for
             index in range(1, self.__m + 1)])
    ...
    def generate(self):
        ...
        print('колличество каналов:{}\n'.format(self.__n))
        # self.show_chart()

        e_prob = self.get_e_prob()
        print('Эмпирическая вероятность отказа:{0}'.format(e_prob[1]))
        ...
        print("Теоретическое среднее время пребывания заявки в очереди: ", self.avg_total_time_e())
